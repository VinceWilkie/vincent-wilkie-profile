...
type RevealDirection = `fade` | `slide-left` | `slide-right`
...
export default function ScrollRevealPage() {
    ...
    useEffect(() => {
        ...
        const observer = new IntersectionObserver(
            ([entry]) => setHideScrollHint(entry.isIntersecting),
    ...
    return (
        ...
        >
            ...
            <button
                onClick={scrollToTop}
                className={`fixed bottom-5 right-5 z-40 p-2.5 rounded-full backdrop-blur-md transition-all duration-300 ${dark ? `bg-gray-800/70 text-gray-300 hover:text-white` : `bg-white/70 text-gray-600 hover:text-gray-900`} ${showBackToTop ? `opacity-100 translate-y-0` : `opacity-0 translate-y-4 pointer-events-none`}`}
            ...
            >
                <div className="flex items-center justify-between p-6">
                    ...
                    <button
                        onClick={() => setDrawerOpen(false)}
                        className={`p-2 rounded-lg transition-colors ${dark ? `text-gray-400 hover:text-white hover:bg-gray-800` : `text-gray-500 hover:text-gray-900 hover:bg-gray-100`}`}
                ...
                <div className="flex-1 flex flex-col gap-1 px-4">
                    {navSections.map((section) => (
                        <button
                            ...
                            className={`text-left px-4 py-3 rounded-lg text-sm font-medium transition-colors ${dark ? `text-gray-300 hover:bg-gray-800 hover:text-white` : `text-gray-700 hover:bg-gray-100 hover:text-gray-900`}`}
                        >
                            {section.label}
                        </button>
                    ))}
                ...
                <div className={`border-t px-6 py-6 space-y-4 ${dark ? `border-gray-800` : `border-gray-200`}`}>
                    <button
                        onClick={toggleTheme}
                        className={`flex items-center gap-3 w-full px-4 py-3 rounded-lg text-sm font-medium transition-colors ${dark ? `text-gray-300 hover:bg-gray-800 hover:text-white` : `text-gray-700 hover:bg-gray-100 hover:text-gray-900`}`}
                    >
                        {dark ? (
                    ...
                    <Link
                        href="/terminal"
                        className="flex items-center gap-3 px-4 py-3 rounded-lg text-sm font-medium transition-colors bg-gray-900 text-white hover:bg-gray-700"
                        onClick={() => setDrawerOpen(false)}
                    >
            ...
            <main className="overflow-x-hidden">
                ...
                {/* ── Hero ── */}
                <section ref={heroRef} id="about" className="min-h-[100lvh] flex flex-col px-4 sm:px-8">
                    <div className="flex-1 flex items-center justify-center">
                        <ScrollReveal direction="fade" className="max-w-3xl mx-auto text-center">
                ...
                {/* Scroll indicator */}
                <div className={`fixed bottom-6 left-1/2 -translate-x-1/2 z-30 flex flex-col items-center gap-1 transition-opacity duration-500 pointer-events-none ${hideScrollHint ? `opacity-0` : `opacity-100`}`}>
                    <span className={`text-xs font-medium tracking-wider uppercase ${dark ? `text-gray-500` : `text-gray-400`}`}>Scroll</span>
                    <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth={2} strokeLinecap="round" strokeLinejoin="round" className={`w-5 h-5 animate-bounce ${dark ? `text-gray-500` : `text-gray-400`}`}>
                        <path d="M6 9l6 6 6-6" />
                    </svg>
                </div>

                {/* ── Skills & Experience ── */}
                <section id="skills" className="py-8 sm:py-12 px-4 sm:px-8">
                    <div ref={skillsHeadingRef}>
                        <ScrollReveal direction="fade" className="max-w-2xl mx-auto text-center mb-6 sm:mb-10">
                            <h2 className={`text-2xl sm:text-4xl font-bold ${dark ? `text-white` : `text-gray-900`}`}>Skills & Experience</h2>
                            <p className={`mt-2 text-sm sm:text-lg ${dark ? `text-gray-400` : `text-gray-500`}`}>What I bring to the table</p>
                        </ScrollReveal>
                    </div>
                    ...
                    <div className="max-w-2xl mx-auto space-y-6 sm:space-y-8">
                        {skills.map((skill, i) => (
                            <ScrollReveal
                                key={skill.name}
                                direction={i % 2 === 0 ? `slide-left` : `slide-right`}
                                delay={i * 50}
                            >
                                <div className={`rounded-2xl p-5 sm:p-10 border shadow-sm ${dark ? `bg-gray-800/50 border-gray-700/50` : `bg-white border-gray-200`}`}>
                ...
                {/* ── Projects ── */}
                <section id="projects" className="py-8 sm:py-12 px-4 sm:px-8">
                    <ScrollReveal direction="fade" className="max-w-2xl mx-auto text-center mb-6 sm:mb-10">
                        <h2 className={`text-2xl sm:text-4xl font-bold ${dark ? `text-white` : `text-gray-900`}`}>Projects</h2>
                        <p className={`mt-2 text-sm sm:text-lg ${dark ? `text-gray-400` : `text-gray-500`}`}>Things I&apos;m building</p>
                    </ScrollReveal>
                ...
                {/* ── Get in Touch ── */}
                <section id="contact" className="py-8 sm:py-12 px-4 sm:px-8">
                    <ScrollReveal direction="fade" className="max-w-2xl mx-auto text-center mb-6 sm:mb-10">
                        <h2 className={`text-2xl sm:text-4xl font-bold ${dark ? `text-white` : `text-gray-900`}`}>Get in Touch</h2>
                        <p className={`mt-2 text-sm sm:text-lg ${dark ? `text-gray-400` : `text-gray-500`}`}>Reach out through any of these channels</p>
                    </ScrollReveal>
                    ...
                    <div className="max-w-lg mx-auto space-y-3 sm:space-y-4">
                        {links.map((link, i) => (
                            <ScrollReveal
                                ...
                                direction={i % 2 === 0 ? `slide-left` : `slide-right`}
                                delay={i * 40}
                                className={`sm:max-w-sm ${i % 2 === 0 ? `sm:mr-auto sm:ml-0` : `sm:ml-auto sm:mr-0`}`}
                            >
                                <Link
                                    href={link.href}
            ...
            {/* Footer */}
            <footer className={`border-t px-4 sm:px-8 py-4 sm:py-6 ${dark ? `border-gray-800` : `border-gray-200`}`}>
